...
void insert_address(struct Env * curenv, uint32 fault_va){
	int inStack=fault_va>=USTACKBOTTOM&&fault_va<USTACKTOP;
	struct Frame_Info * frame=NULL;
	int x=allocate_frame(&frame);
	map_frame(curenv->env_page_directory,frame,(void*)fault_va,PERM_PRESENT|PERM_USER|PERM_WRITEABLE);
	x=pf_read_env_page(curenv,(void*)fault_va);
	if(x==E_PAGE_NOT_EXIST_IN_PF){
		if(!inStack){
			panic("address %x : Not in Stack and not in PageFile",fault_va);
			return;
		}
		x=pf_add_empty_env_page(curenv,fault_va,1);
		if(x==E_NO_PAGE_FILE_SPACE){
			free_frame(frame);
...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	uint32 WSsize=curenv->page_WS_max_size;
	do{
		if(env_page_ws_is_entry_empty(curenv,curenv->page_last_WS_index))break;
		curenv->page_last_WS_index=(curenv->page_last_WS_index+1)%WSsize;
	}while(curenv->page_last_WS_index!=first);

	if(env_page_ws_is_entry_empty(curenv,curenv->page_last_WS_index)){
		insert_address(curenv,fault_va);
		return;
	}
	...
	while(victum == -1){
		if(!mode){
			do{
				...
				uint32 permition=pt_get_page_permissions(curenv,va);
				if(!((permition&PERM_USED)||(permition&PERM_MODIFIED))){
					victum=curenv->page_last_WS_index;
					break;
				}
				curenv->page_last_WS_index=(curenv->page_last_WS_index+1)%WSsize;
			}while(curenv->page_last_WS_index!=first);
	...
	uint32 va=env_page_ws_get_virtual_address(curenv,victum);
	uint32 permition=pt_get_page_permissions(curenv,va);
	struct Frame_Info * frame=NULL;
	uint32* ptr_table=NULL;
	frame=get_frame_info(curenv->env_page_directory,(void*)va,&ptr_table);
	if(permition&PERM_MODIFIED){
		int x=pf_update_env_page(curenv,(void*)va,frame);
	}
	free_frame(frame);
	unmap_frame(curenv->env_page_directory,(void*)va);
	env_page_ws_clear_entry(curenv,victum);
	insert_address(curenv,fault_va);
