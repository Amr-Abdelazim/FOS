...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	uint32 *ptr=(uint32*)kmalloc(PAGE_SIZE);
	for(int i=0;i<PAGE_SIZE/4;i++){
		ptr[i]=0;
	}
	uint32 phys=(uint32)kheap_physical_address((uint32)ptr);
	uint32 entry=CONSTRUCT_ENTRY(phys,PERM_USER|PERM_PRESENT|PERM_WRITEABLE);
	ptr_page_directory[PDX(virtual_address)]=entry;
	tlbflush();
	return ptr;
}
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	for(uint32 i=virtual_address;i<virtual_address+size*PAGE_SIZE;i+=PAGE_SIZE){
		pf_remove_env_page(e,i);
	}

	for(int i=0;i<e->page_WS_max_size;i++){
		uint32 va=env_page_ws_get_virtual_address(e,i);
		if(va>=virtual_address&&va<virtual_address+size*PAGE_SIZE){
